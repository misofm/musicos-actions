...
fun setup(sc: &mut Scenario): ID {
    ...
    let created = event::events_by_type<action::CompositionRoutedStakeCreatedEvent<RECORDING_SHARE, COMPOSITION_SHARE>>();
    assert_eq!(created.length(), 1);
    let (_, _, _, routed_id, stake_id, _, principal, registrations) = action::created_event_fields(&created[0]);
...
fun admin_exits(sc: &mut Scenario) {
    ...
    let (_, _, _, unregistered_routed_id, unregistered_stake_id, _, unregistered_principal, before, after, _, _, pool_balance, index, debt, carry, deposits, forfeited) =
        action::unregistered_event_fields(&unregistered[0]);
    assert_eq!(unregistered_routed_id, object::id(&routed).to_address());
    assert_eq!(unregistered_stake_id, stake_id);
    assert_eq!(unregistered_principal, COMPOSITION_CUT); assert_eq!(before, 1); assert_eq!(after, 0);
    assert!(index > 0); assert!(debt > 0); assert!(forfeited < 1_000_000_000_000_000_000u256);
    assert_eq!(pool_balance, recording_pool.balance().value());
