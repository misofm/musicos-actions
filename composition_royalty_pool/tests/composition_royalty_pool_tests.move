...
fun new_pool_is_returned_unshared_with_exact_parent_and_event() {
    ...
    assert_eq!(parent_id, composition_id.to_address());
    let action_events =
...
fun duplicate_pool_derivation_claim_aborts() {
    ...
    let (mut composition, admin_cap) = fixture(ctx);
    let (share_currency, supply) = test_share::bootstrap_currency(ctx);
    let first = action::new_pool<COMPOSITION_SHARE, CURRENCY>(
        &mut composition, &admin_cap, &share_currency,
    );
    let second = action::new_pool<COMPOSITION_SHARE, CURRENCY>(
        &mut composition, &admin_cap, &share_currency,
    );
    destroy(second);
    destroy(first);
...
fun redeem_all_is_an_idempotent_no_op_without_settled_funds() {
    ...
    action::redeem_all_and_deposit(&mut composition, &admin_cap, &mut pool, &root);
    action::redeem_all_and_deposit(&mut composition, &admin_cap, &mut pool, &root);
...
fun settled_value_helper_is_a_silent_no_op_at_zero() {
    ...
    pool.register_stake(&mut holder);
    let events_before = event::num_events();
    action::redeem_settled_value_and_deposit_for_testing(&mut composition, &admin_cap, &mut pool, 0);
    assert_eq!(event::num_events(), events_before);
    assert_eq!(pool.cumulative_deposits(), 0);
    pool.unregister_stake(&mut holder);
    balance::destroy_for_testing(stake::destroy(holder));
...
fun zero_stakers_is_a_no_op_and_funds_remain_redeemable_after_a_stake_registers() {
    ...
    assert_eq!(pool.cumulative_deposits(), 0);
    assert_eq!(
        event::events_by_type<RoyaltyDepositedEvent<COMPOSITION_SHARE, CURRENCY>>().length(),
        0,
    );
    ...
    let action_events =
        event::events_by_type<action::CompositionFundsDepositedEvent<COMPOSITION_SHARE, CURRENCY>>();
    assert_eq!(action_events.length(), 1);
    let (_, _, _, amount, balance_before, balance_after, staked, _, _, _, _, deposits_before, deposits_after) =
        action::funds_deposited_event_fields(&action_events[0]);
    assert_eq!(amount, 321);
...
/// Composition in one PTB withdraws the snapshot again and the whole transaction
...
fun second_redeem_all_in_the_same_tx_sees_the_vm_zero_snapshot() {
    ...
    scenario.next_tx(@0xB);
    action::redeem_settled_value_and_deposit_for_testing(&mut composition, &admin_cap, &mut pool, 500);
...
/// include each Composition at most once per PTB.
#[test]
...
fun u64_max_snapshot_deposits_without_abort() {
    ...
    action::redeem_settled_value_and_deposit_for_testing(&mut composition, &admin_cap, &mut pool, max);
...
/// Batch safety: three Compositions cranked in one transaction. One has an
...
fun batch_with_zero_snapshot_and_zero_staker_items_still_deposits_the_others() {
    ...
    scenario.next_tx(@0xB);
    let root = scenario.take_shared<AccumulatorRoot>();
    action::redeem_settled_value_and_deposit_for_testing(&mut composition_a, &cap_a, &mut pool_a, 1_000);
    action::redeem_all_and_deposit(&mut composition_b, &cap_b, &mut pool_b, &root);
    action::redeem_settled_value_and_deposit_for_testing(&mut composition_c, &cap_c, &mut pool_c, 250);
    ...
    scenario.next_tx(@0xC);
    let mut holder_c = stake::new(balance::create_for_testing<COMPOSITION_SHARE>(10), scenario.ctx());
