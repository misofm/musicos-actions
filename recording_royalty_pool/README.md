...
The no-op holds only across commits: the snapshot is written solely by consensus settlement and is constant within a commit, so redeeming the same Recording twice in one PTB, or from two crankers in the same commit, withdraws the snapshot twice and the network fails that whole transaction with `InsufficientFundsForWithdraw` (not a Move abort). A crank must include each Recording at most once per PTB and retry that status next commit.
